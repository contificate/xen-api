...
let test_cache_similar_passwords () =
  ...
  let cache = Cache.create ~size:1 ~expiry:Mtime.Span.(10 * s) in
...
let test_cache_expiration () =
  ...
  let expiry = Mtime.Span.(expiry_seconds * s) in
  let cache = Cache.create ~size:100 ~expiry in
...
let test_cache_updates_duplicates () =
  ...
  let cache = Cache.create ~size:count ~expiry in
