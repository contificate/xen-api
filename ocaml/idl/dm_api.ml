...
let get_obj_by_name (system, _) ~objname:name =
  ...
  | _ :: _ ->
      failwith
        (Printf.sprintf "Multiple instances of name [%s] found in system" name)
  | [] ->
...
let filter (obj : obj -> bool) (field : field -> bool)
  ...
  let system =
    List.filter (fun obj -> obj.messages <> [] || obj.contents <> []) system
  in
  rebuild system relations

let filter_by ?(obj = fun _ -> true) ?(field = fun _ -> true)
    ?(msg = fun _ -> true) =
  filter obj field msg
...
let check api emergency_calls =
  ...
  *)
  let rec flatten_fields fs acc =
    ...
    | Namespace (_, internal_fs) :: fs ->
        flatten_fields fs (flatten_fields internal_fs acc)
  in
  let _ =
    ...
    List.iter obj (objects_of_api api)
  in
  ...
  let _ =
    List.iter
      (fun obj ->
        List.iter
          (fun msg ->
            if msg.msg_tag = Custom && not (are_in_vsn_order msg.msg_params)
            then
              failwith
                (Printf.sprintf
